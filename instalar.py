...
call(["sudo apt-get install \ ca-certificates \ curl \ gnupg \ lsb-release"], shell=True)
...
call(["echo \ "+r'"'+"deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu \ $(lsb_release -cs) stable"+r'"'+" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null"], shell=True)
call(["echo 'y' | sudo apt-get update"], shell=True)
